...
// =================================================================================
const THEME_VARS = `
  /* LIGHT — Premium, Accessible, and High-Contrast */
  [data-theme="light"] {
    --bg-main: radial-gradient(circle at top, #f8fafc 0%, #e2e8f0 100%);
    --header-bg: rgba(255, 255, 255, 0.9);
    --header-border: rgba(0, 0, 0, 0.05);
    
    --card-bg: #ffffff;
    --card-border: rgba(0, 0, 0, 0.04);
    --card-shadow: 0 10px 40px -10px rgba(0, 0, 0, 0.08); /* Playbook 4: Floating cards */
    --card-hover-shadow: 0 20px 40px -10px rgba(0, 0, 0, 0.12);
    
    --text-main: #111827;
    --text-muted: #475569; /* Playbook 3: Deepened secondary text */
    --text-dim: #64748b;
    --text-inverse: #ffffff;
    
    /* Playbook 1: Saturated Top Summary Contrast */
    --stat-on-color: #065f46;   --stat-on-bg: #d1fae5;
    --stat-run-color: #047857;  --stat-run-bg: #d1fae5;
    --stat-idle-color: #b45309; --stat-idle-bg: #fef3c7;
    --stat-off-color: #334155;  --stat-off-bg: #f1f5f9;
    --stat-tot-color: #1d4ed8;  --stat-tot-bg: #dbeafe;
    ...
    --glow-running: 0 5px 15px rgba(16, 185, 129, 0.15);
    --glow-idle: 0 5px 15px rgba(245, 158, 11, 0.15);

    /* Badges - Playbook 5: Subtle BG, No Borders */
    --badge-border: transparent;
    --badge-run-bg: #d1fae5;    --badge-run-text: #047857;
    --badge-idle-bg: #fef3c7;   --badge-idle-text: #b45309;
    --badge-off-bg: #f1f5f9;    --badge-off-text: #475569;

    /* Playbook 2: Live Idle Alerts (Soft BG, Hard Text) */
    --alert-pulse-bg-1: #fee2e2;
    --alert-pulse-bg-2: #fecaca;
    --alert-pulse-text: #991b1b;
    --alert-pulse-border: #f87171;

    --alert-off-bg-1: #f1f5f9;
    --alert-off-bg-2: #e2e8f0;
    --alert-off-text: #334155;
    --alert-off-border: #cbd5e1;

    --inner-box-bg: #f8fafc;
    --inner-box-border: rgba(0, 0, 0, 0.03);

    /* Modal & Dropdown */
    --modal-bg: #ffffff;
    --modal-header-end: #f1f5f9;
    --modal-input-bg: #f8fafc;
    --dropdown-bg: #ffffff;
    --dropdown-text: #111827;
    --dropdown-border: rgba(0, 0, 0, 0.1);
    --dropdown-hover-bg: #fef3c7;
    --dropdown-hover-text: #b45309;
    --dropdown-icon: #b45309;
  ...
  [data-theme="dark"] {
    --bg-main: radial-gradient(circle at top, #1e293b 0%, #020617 100%);
    ...
    --stat-on-color: #34d399;   --stat-on-bg: rgba(16, 185, 129, 0.05);
    --stat-run-color: #10b981;  --stat-run-bg: rgba(16, 185, 129, 0.1);
    --stat-idle-color: #f59e0b; --stat-idle-bg: rgba(245, 158, 11, 0.1);
    --stat-off-color: #64748b;  --stat-off-bg: rgba(100, 116, 139, 0.1);
    --stat-tot-color: #3b82f6;  --stat-tot-bg: rgba(59, 130, 246, 0.1);

    --status-running: #10b981;
    --status-idle: #f59e0b;
    --status-offline: #475569;
    
    --glow-running: 0 0 15px rgba(16, 185, 129, 0.4);
    --glow-idle: 0 0 15px rgba(245, 158, 11, 0.4);

    --badge-border: 1px solid;
    --badge-run-bg: rgba(16, 185, 129, 0.2);  --badge-run-text: #10b981;
    --badge-idle-bg: rgba(245, 158, 11, 0.2); --badge-idle-text: #f59e0b;
    --badge-off-bg: rgba(71, 85, 105, 0.2);   --badge-off-text: #475569;

    --alert-pulse-bg-1: #9a3412;
    --alert-pulse-bg-2: #7c2d12;
    --alert-pulse-text: #fff;
    --alert-pulse-border: rgba(249, 115, 22, 0.3);

    --alert-off-bg-1: rgba(71, 85, 105, 0.1);
    --alert-off-bg-2: rgba(71, 85, 105, 0.3);
    --alert-off-text: #f8fafc;
    --alert-off-border: rgba(71, 85, 105, 0.4);

    --inner-box-bg: rgba(2, 6, 23, 0.4);
    --inner-box-border: rgba(255, 255, 255, 0.02);

    --modal-bg: #0f172a;
    --modal-header-end: #0f172a;
    --modal-input-bg: rgba(16, 185, 129, 0.05);
    --dropdown-bg: #1e293b;
    --dropdown-text: #94a3b8;
    --dropdown-border: rgba(245, 158, 11, 0.4);
    --dropdown-hover-bg: rgba(245, 158, 11, 0.15);
    --dropdown-hover-text: #fbbf24;
    --dropdown-icon: #fcd34d;
  ...
  return (
    <div ref={dropdownRef} style={{ position: 'relative', width: '100%' }}>
      <div
        ...
        style={{
          width: '100%', padding: '14px 18px', borderRadius: '12px',
          background: 'var(--dropdown-bg)', color: value ? 'var(--text-main)' : 'var(--text-muted)',
          border: `1px solid ${isOpen ? 'var(--dropdown-icon)' : 'var(--dropdown-border)'}`,
          boxShadow: isOpen ? '0 0 0 3px rgba(245, 158, 11, 0.15)' : 'var(--card-shadow)',
          fontSize: '15px', fontWeight: '600', cursor: 'pointer',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          transition: 'all 0.3s ease', userSelect: 'none'
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          {value && icon && CATEGORY_ICONS[value] && (
             <i className={CATEGORY_ICONS[value]} style={{ color: 'var(--dropdown-icon)' }}></i>
          )}
          <span>{value || placeholder}</span>
        </div>
        <div style={{
           transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)',
           transition: 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
           color: isOpen ? 'var(--dropdown-icon)' : 'var(--text-muted)',
           display: 'flex', alignItems: 'center'
        }}>
          ▼
        </div>
      ...
      {isOpen && (
        <div style={{
          position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px',
          background: 'var(--modal-bg)', border: '1px solid var(--dropdown-border)',
          ...
          {options.map((opt, idx) => (
            <div
              ...
              onMouseEnter={(e) => { 
                ...
                e.currentTarget.style.color = 'var(--dropdown-hover-text)'; 
                e.currentTarget.style.paddingLeft = '24px';
              }}
              onMouseLeave={(e) => { 
                e.currentTarget.style.background = 'transparent'; 
                e.currentTarget.style.color = value === opt ? 'var(--dropdown-hover-text)' : 'var(--text-main)'; 
                e.currentTarget.style.paddingLeft = '18px';
              }}
              style={{
                padding: '14px 18px', cursor: 'pointer', fontSize: '14px', fontWeight: '500',
                color: value === opt ? 'var(--dropdown-hover-text)' : 'var(--text-main)',
                background: value === opt ? 'var(--dropdown-hover-bg)' : 'transparent',
                borderBottom: idx !== options.length - 1 ? '1px solid var(--dropdown-border)' : 'none',
                transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', gap: '10px'
              }}
            >
              {icon && CATEGORY_ICONS[opt] && (
...
export default function Plant1Live() {
  ...
  // Sync theme with global layout (No logic changes, just observer injection)
  const [theme, setTheme] = useState(() => {
    if (typeof window === "undefined") return "dark";
    return window.localStorage.getItem("atomone-theme") || "dark";
  });
  ...
  useEffect(() => {
    ...
    const handleStorage = () => setTheme(window.localStorage.getItem("atomone-theme") || "dark");
  ...
  const getMachineColor = (machine) => {
    if (!machine.machine_on) return 'var(--status-offline)'; 
    if (machine.is_producing) return 'var(--status-running)'; 
    return 'var(--status-idle)'; 
  };

  const getMachineGlow = (machine) => {
    if (!machine.machine_on) return 'none'; 
    if (machine.is_producing) return 'var(--glow-running)';
    return 'var(--glow-idle)';
  };

  const getBadgeStyle = (machine) => {
    if (!machine.machine_on) return { bg: 'var(--badge-off-bg)', text: 'var(--badge-off-text)' };
    if (machine.is_producing) return { bg: 'var(--badge-run-bg)', text: 'var(--badge-run-text)' };
    return { bg: 'var(--badge-idle-bg)', text: 'var(--badge-idle-text)' };
  }
  ...
  // Check hour change
   ...
   if (loading && machines.length === 0) {
     return (
       <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', backgroundColor: 'var(--modal-bg)', color: 'var(--status-running)', fontSize: '24px', fontWeight: 'bold' }}>
         <div className="loader" style={{ marginBottom: '20px', border: '4px solid var(--dropdown-border)', borderTop: '4px solid var(--status-running)', borderRadius: '50%', width: '50px', height: '50px', animation: 'spin 1s linear infinite' }}></div>
         Loading Plant Dashboard...
         <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
       </div>
     );
   ...
   return (
     <div data-theme={theme} style={{
       ...
       <style>{`
         @keyframes hardPulse { 
           0%, 100% { opacity: 1; background-color: var(--alert-pulse-bg-1); } 
           50% { opacity: 0.9; background-color: var(--alert-pulse-bg-2); box-shadow: 0 0 20px var(--alert-pulse-border); } 
         }
         @keyframes offlinePulse { 
           0%, 100% { background-color: var(--alert-off-bg-1); border-color: var(--alert-off-border); } 
           50% { background-color: var(--alert-off-bg-2); box-shadow: 0 0 15px var(--alert-off-border); border-color: var(--alert-off-border); } 
         }
         @keyframes alertTextPulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.6; transform: scale(1.02); } }
         @keyframes lunchBannerPulse { 0%, 100% { background-color: rgba(59, 130, 246, 0.15); border-color: rgba(96, 165, 250, 0.4); } 50% { background-color: rgba(59, 130, 246, 0.3); border-color: rgba(96, 165, 250, 0.8); box-shadow: 0 0 15px rgba(59, 130, 246, 0.3); } }
         @keyframes slideDownDropdown { from { opacity: 0; transform: scaleY(0.95); } to { opacity: 1; transform: scaleY(1); } }
         .card-hover:hover { transform: translateY(-6px); box-shadow: var(--card-hover-shadow) !important; }
         ::-webkit-scrollbar { width: 8px; } ::-webkit-scrollbar-track { background: var(--bg-main); } ::-webkit-scrollbar-thumb { background: var(--text-dim); border-radius: 4px; }
         .timeline-container { position: relative; padding: 20px 0; } .timeline-line { position: absolute; left: 24px; top: 0; bottom: 0; width: 2px; background: var(--timeline-line); }
         .timeline-item { position: relative; padding-left: 60px; margin-bottom: 30px; } .timeline-icon { position: absolute; left: 12px; top: 0; width: 26px; height: 26px; border-radius: 50%; background: var(--card-bg); border: 2px solid; display: flex; align-items: center; justify-content: center; font-size: 12px; z-index: 2; }
         .timeline-content { background: var(--timeline-bg); border: 1px solid var(--card-border); padding: 15px; border-radius: 12px; }
       `}</style>
 
       {/* HEADER SECTION */}
       <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '30px', padding: '24px', background: 'var(--header-bg)', borderRadius: '20px', border: '1px solid var(--header-border)', boxShadow: 'var(--card-shadow)' }}>
         <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
             <div style={{ width: '12px', height: '40px', background: 'var(--status-running)', borderRadius: '6px', boxShadow: 'var(--glow-running)' }}></div>
             <h1 style={{ margin: 0, fontSize: '32px', color: 'var(--text-main)', fontWeight: '800' }}>Plant 1 <span style={{ color: 'var(--status-running)' }}>Live Status</span></h1>
           </div>
           <div style={{ display: 'flex', gap: '12px', fontSize: '14px', fontWeight: '600' }}>
             <div style={{ padding: '8px 16px', background: 'var(--stat-run-bg)', border: '1px solid var(--badge-border)', borderRadius: '10px', color: 'var(--stat-run-color)' }}>Shift: {currentShift}</div>
             <div style={{ padding: '8px 16px', background: 'var(--inner-box-bg)', borderRadius: '10px', border: '1px solid var(--inner-box-border)', color: 'var(--text-main)' }}>Hour: {currentHour.toString().padStart(2, '0')}:xx</div>
           </div>
         </div>
         <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px' }}>
           {[
             { label: 'ON MACHINES', value: onMachines, color: 'var(--stat-on-color)', bg: 'var(--stat-on-bg)' },
             { label: 'RUNNING', value: runningMachines, color: 'var(--stat-run-color)', bg: 'var(--stat-run-bg)' },
             { label: 'IDLE', value: idleMachines, color: 'var(--stat-idle-color)', bg: 'var(--stat-idle-bg)' },
             { label: 'OFFLINE', value: offlineMachines, color: 'var(--stat-off-color)', bg: 'var(--stat-off-bg)' },
             { label: 'TOTAL', value: totalMachines, color: 'var(--stat-tot-color)', bg: 'var(--stat-tot-bg)' }
           ].map((stat, i) => (
             <div key={i} style={{ textAlign: 'center', padding: '16px 10px', background: stat.bg, borderRadius: '16px', border: `1px solid var(--card-border)` }}>
               <div style={{ fontSize: '32px', fontWeight: '900', color: stat.color, lineHeight: '1' }}>{stat.value}</div>
               <div style={{ fontSize: '11px', color: stat.color, opacity: 0.8, textTransform: 'uppercase', marginTop: '8px', fontWeight: '800' }}>{stat.label}</div>
             </div>
           ))}
         </div>
       </div>
       ...
       <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '24px' }}>
         {sortedMachines.map((machine) => {
           const liveIdle = liveIdleSeconds[machine.machine_no] || 0;
           const statusColor = getMachineColor(machine);
           const badgeStyle = getBadgeStyle(machine);
           const isOffline = !machine.machine_on;
           const isRecentlyOffline = isOffline && machine.offline_duration_minutes !== null && machine.offline_duration_minutes <= 15;
           const showIdlePulse = !isCurrentlyLunch && (!machine.is_producing);
           const hasLoggedReason = reasonLoggedStates[machine.machine_no];
           ...
           return (
             <div key={machine.machine_no} className="card-hover" onClick={() => handleMachineClick(machine)} style={{ position: 'relative', background: 'var(--card-bg)', border: `1px solid var(--card-border)`, borderTop: `4px solid ${statusColor}`, borderRadius: '20px', padding: '20px', cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', boxShadow: `var(--card-shadow), ${getMachineGlow(machine)}` }}>
               <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                 <div style={{ fontSize: '24px', fontWeight: '900', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '8px' }}>Machine {machine.machine_no}</div>
                 <div style={{ fontSize: '11px', padding: '6px 12px', background: badgeStyle.bg, color: badgeStyle.text, borderRadius: '20px', fontWeight: '800', border: `var(--badge-border) ${statusColor}40` }}>{getMachineStatus(machine)}</div>
               </div>
 
               {isRecentlyOffline && (
                 <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', color: 'var(--text-muted)', fontWeight: '700', letterSpacing: '2px', fontSize: '11px', marginBottom: '16px', textTransform: 'uppercase' }}>
                   <span style={{flex: 1, height: '1px', borderTop: '1px dashed var(--text-dim)'}}></span>Recently Offline<span style={{flex: 1, height: '1px', borderTop: '1px dashed var(--text-dim)'}}></span>
                 </div>
               )}
               ...
               <div style={{ textAlign: 'center', padding: '24px 10px', background: 'var(--inner-box-bg)', borderRadius: '16px', marginBottom: '16px', border: '1px solid var(--inner-box-border)' }}>
                 <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: '800', marginBottom: '10px' }}>Current Hour Production</div>
                 <div style={{ fontSize: '56px', fontWeight: '900', color: statusColor, lineHeight: '1', textShadow: `0 0 20px ${statusColor}40` }}>{machine.current_hour_count || 0}</div>
               </div>
 
               <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
                 <div style={{ padding: '12px', background: 'var(--stat-run-bg)', borderRadius: '12px' }}><div style={{ fontSize: '11px', color: 'var(--stat-run-color)', opacity: 0.8, fontWeight: '700' }}>Machine ON at</div><div style={{ fontSize: '16px', fontWeight: 'bold', color: 'var(--stat-run-color)' }}>{machine.on_since || '--:--'}</div></div>
                 <div style={{ padding: '12px', background: 'var(--stat-tot-bg)', borderRadius: '12px' }}><div style={{ fontSize: '11px', color: 'var(--stat-tot-color)', opacity: 0.8, fontWeight: '700' }}>Start Production at</div><div style={{ fontSize: '16px', fontWeight: 'bold', color: 'var(--stat-tot-color)' }}>{machine.first_count_at || '--:--'}</div><div style={{ fontSize: '11px', color: 'var(--stat-tot-color)', opacity: 0.8, fontWeight: '600' }}>Idle: {machine.time_to_first_count || 0} min</div></div>
               </div>
 
               {showIdlePulse && liveIdle > 0 && (
                 <div style={{ padding: '16px', borderRadius: '12px', marginBottom: '16px', textAlign: 'center', animation: isOffline ? 'offlinePulse 2s infinite' : 'hardPulse 2s infinite', border: '1px solid transparent' }}>
                   <div style={{ fontSize: '11px', color: isOffline ? 'var(--alert-off-text)' : 'var(--alert-pulse-text)', fontWeight: '800', marginBottom: '8px' }}>{isOffline ? '⚠️ OFFLINE TIMER' : '⚠️ LIVE IDLE'}</div>
                   <div style={{ fontSize: '32px', fontWeight: '900', color: isOffline ? 'var(--alert-off-text)' : 'var(--alert-pulse-text)', lineHeight: '1' }}>{formatLiveIdle(liveIdle)}</div>
                   {isOffline && (<div style={{ fontSize: '12px', color: 'var(--text-dim)', marginTop: '8px', fontWeight: '600' }}>Offline Since: <span style={{color: 'var(--alert-off-text)'}}>{machine.offline_since || 'Shift Start'}</span></div>)}
                   
                   {!hasLoggedReason ? (
                     <div style={{ marginTop: '12px', padding: '6px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', letterSpacing: '0.5px', color: '#ef4444', background: 'rgba(239, 68, 68, 0.15)', border: '1px solid rgba(239, 68, 68, 0.3)', animation: 'alertTextPulse 1.5s infinite' }}>
                         ⚠️ Plz fill the information
                     </div>
                   ) : (
                     <div style={{ marginTop: '12px', padding: '6px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', letterSpacing: '0.5px', color: 'var(--status-idle)', background: 'var(--badge-idle-bg)', border: '1px solid var(--badge-idle-text)' }}>
                         ✅ Reason Submitted
                     </div>
                   )}
                 </div>
               ...
               <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                 <div style={{ padding: '12px', background: 'var(--inner-box-bg)', borderRadius: '12px', border: '1px solid var(--inner-box-border)' }}><div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: '600' }}>Total Idle (Hour)</div><div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--status-idle)' }}>{machine.total_shift_idle_time || 0} min</div></div>
                 <div style={{ padding: '12px', background: 'var(--inner-box-bg)', borderRadius: '12px', border: '1px solid var(--inner-box-border)' }}><div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: '600' }}>Shut Height</div><div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--text-main)' }}>{typeof machine.shut_height === 'number' ? machine.shut_height.toFixed(2) : machine.shut_height || '--'}</div></div>
                 <div style={{ padding: '12px', background: 'var(--inner-box-bg)', borderRadius: '12px', border: '1px solid var(--inner-box-border)' }}><div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: '600' }}>Last Hour</div><div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--text-dim)' }}>{machine.last_hour_count || 0}</div></div>
                 <div style={{ padding: '12px', background: 'var(--inner-box-bg)', borderRadius: '12px', border: '1px solid var(--inner-box-border)' }}><div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: '600' }}>Cumulative</div><div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--status-running)' }}>{machine.cumulative_count || 0}</div></div>
               </div>
             </div>
           );
         })}
       ...
       {/* MODAL POPUP */}
       {selectedMachine && (
         <div onClick={closeModal} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(2, 6, 23, 0.6)', backdropFilter: 'blur(8px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
           <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: 'var(--modal-bg)', borderRadius: '24px', padding: 0, maxWidth: '800px', width: '100%', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.4)', border: `1px solid var(--card-border)` }}>
             <div style={{ padding: '30px', background: `linear-gradient(135deg, ${getMachineColor(selectedMachine)} 0%, var(--modal-header-end) 150%)`, borderTopLeftRadius: '24px', borderTopRightRadius: '24px' }}>
               <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                 <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                   <div style={{ fontSize: '32px', fontWeight: '900', color: 'var(--text-inverse)', textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>Machine {selectedMachine.machine_no}</div>
                   {!showHistoryView && <div style={{ padding: '8px 20px', backgroundColor: 'rgba(0,0,0,0.2)', color: 'white', borderRadius: '30px', fontWeight: '800' }}>{getMachineStatus(selectedMachine)}</div>}
                   <div title={showHistoryView ? "Back to Dashboard" : "View Timeline History"} style={{ position: 'relative', cursor: 'pointer', padding: '10px 15px', backgroundColor: showHistoryView ? '#10b981' : 'rgba(0,0,0,0.2)', color: 'white', borderRadius: '30px', fontWeight: '700', border: '1px solid rgba(255,255,255,0.2)', transition: 'all 0.2s', display: 'flex', gap: '8px', alignItems: 'center' }} onClick={toggleHistoryView}>
                     <span style={{ fontSize: '18px' }}>{showHistoryView ? "🔙" : "📅"}</span><span>{showHistoryView ? "Live View" : "History"}</span>
                   </div>
                 </div>
                 <button onClick={closeModal} style={{ background: 'rgba(0,0,0,0.2)', color: 'white', border: 'none', borderRadius: '50%', width: '44px', height: '44px', fontSize: '24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
               </div>
             </div>
             ...
             {showHistoryView ? (
               <div style={{ padding: '30px' }}>
                 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                     <h2 style={{ margin: 0, color: 'var(--text-main)' }}>Events Timeline</h2>
                     <input type="date" value={historyDate} onChange={(e) => { setHistoryDate(e.target.value); fetchMachineHistory(selectedMachine.machine_no, e.target.value); }} style={{ padding: '10px 15px', borderRadius: '8px', border: '1px solid var(--dropdown-border)', background: 'var(--dropdown-bg)', color: 'var(--text-main)', fontFamily: 'inherit', fontWeight: 'bold' }} />
                 </div>
                 {historyError ? (
                     <div style={{ textAlign: 'center', padding: '50px', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '16px', border: '1px dashed rgba(239, 68, 68, 0.3)' }}><div style={{ fontSize: '40px', marginBottom: '10px' }}>⚠️</div><h3 style={{ color: '#ef4444', margin: 0 }}>API Connection Error</h3><p style={{ color: '#fca5a5', fontSize: '14px' }}>{historyError}</p></div>
                 ) : historyLoading ? (
                     <div style={{ textAlign: 'center', padding: '50px', color: 'var(--text-muted)' }}><div className="loader" style={{ margin: '0 auto 20px', border: '3px solid var(--card-border)', borderTop: '3px solid var(--status-running)', borderRadius: '50%', width: '40px', height: '40px', animation: 'spin 1s linear infinite' }}></div>Loading Timeline Data...</div>
                 ) : historyData.length === 0 ? (
                     <div style={{ textAlign: 'center', padding: '50px', background: 'var(--inner-box-bg)', borderRadius: '16px', border: '1px dashed var(--inner-box-border)' }}><div style={{ fontSize: '40px', marginBottom: '10px' }}>📭</div><h3 style={{ color: 'var(--text-muted)', margin: 0 }}>No Events Found</h3><p style={{ color: 'var(--text-dim)', fontSize: '14px' }}>There are no recorded events for this machine on {historyDate}</p></div>
                 ) : (
                     <div className="timeline-container">
                         <div className="timeline-line"></div>
                         {historyData.map((event, index) => {
                             ...
                             return (
                                 <div key={index} className="timeline-item">
                                     <div className="timeline-icon" style={{ borderColor: color }}>{eventIcons[event.type] || '📌'}</div>
                                     <div className="timeline-content" style={{ borderLeft: `3px solid ${color}` }}>
                                         <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ fontWeight: 'bold', color: color, fontSize: '16px' }}>{event.type.replace(/_/g, ' ')}</span><span style={{ color: 'var(--text-muted)', fontSize: '14px', background: 'var(--inner-box-bg)', border: '1px solid var(--inner-box-border)', padding: '4px 10px', borderRadius: '20px' }}>{event.time}</span></div>
                                         <div style={{ color: 'var(--text-main)', fontSize: '14px', lineHeight: '1.5', fontWeight: '500' }}>{event.details || (isOff ? 'Machine detected as Offline/No Signal' : 'Machine Power/Signal Restored')}</div>
                                         <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginTop: '10px', display: 'flex', gap: '15px', fontWeight: '600' }}><span>Shift: {event.shift}</span><span>System Time: {event.raw_time}</span></div>
                                     </div>
                                 </div>
                             );
                         })}
             ...
             <div style={{ padding: '30px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
               ...
               {!selectedMachine.is_producing && (
                 <div style={{ background: 'var(--stat-idle-bg)', padding: '24px', borderRadius: '20px', border: '1px solid var(--stat-idle-color)', boxShadow: '0 4px 15px rgba(245, 158, 11, 0.05)' }}>
                   <h3 style={{ margin: '0 0 24px 0', color: 'var(--stat-idle-color)', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '20px' }}>
                     <div style={{ background: 'rgba(245, 158, 11, 0.2)', width: '36px', height: '36px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="bi bi-exclamation-triangle-fill" style={{ fontSize: '18px' }}></i></div>
                     Log Downtime Reason
                     <span style={{ fontSize: '12px', padding: '4px 10px', borderRadius: '6px', background: selectedMachine.machine_on ? 'var(--badge-run-bg)' : 'var(--badge-off-bg)', color: selectedMachine.machine_on ? 'var(--badge-run-text)' : 'var(--badge-off-text)', border: `1px solid ${selectedMachine.machine_on ? 'var(--badge-run-text)' : 'var(--badge-off-text)'}`, marginLeft: 'auto' }}>
                       {selectedMachine.machine_on ? 'ONLINE (IDLE)' : 'OFFLINE'}
                     </span>
                   </h3>
                   ...
                   {!reasonLoggedStates[selectedMachine.machine_no] ? (
                     <>
                       <div style={{ marginBottom: '16px' }}>
                         <label style={{ fontSize: '11px', color: 'var(--stat-idle-color)', fontWeight: '800', textTransform: 'uppercase', marginBottom: '8px', display: 'block', letterSpacing: '1.5px' }}>1. Select Category</label>
                         <CustomDropdown value={idleCategory} placeholder="-- Choose Category --" icon={true} options={Object.keys(IDLE_REASONS)} onChange={(val) => { setIdleCategory(val); if (val !== 'Other') setCustomCategory(''); if (val === 'Other') { setIdleSubReason('Other'); } else { setIdleSubReason(''); } setIdleRemarks(''); }} />
                       </div>
 
                       {idleCategory === 'Other' && (
                         <div style={{ marginBottom: '16px', animation: 'slideDownDropdown 0.3s ease-out forwards' }}>
                           <label style={{ fontSize: '11px', color: 'var(--status-running)', fontWeight: '800', textTransform: 'uppercase', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '5px', letterSpacing: '1.5px' }}><i className="bi bi-pencil-square"></i> Please Specify Custom Category</label>
                           <input type="text" placeholder="Type custom category name..." value={customCategory} onChange={(e) => setCustomCategory(e.target.value)} style={{ width: '100%', padding: '14px 18px', borderRadius: '12px', background: 'var(--modal-input-bg)', color: 'var(--text-main)', border: '1px solid var(--status-running)', fontSize: '14px', outline: 'none' }} />
                         </div>
                       )}
 
                       {idleCategory && (
                         <div style={{ marginBottom: '16px', animation: 'slideDownDropdown 0.3s ease-out forwards' }}>
                           <label style={{ fontSize: '11px', color: 'var(--stat-idle-color)', fontWeight: '800', textTransform: 'uppercase', marginBottom: '8px', display: 'block', letterSpacing: '1.5px' }}>2. Specific Reason</label>
                           <CustomDropdown value={idleSubReason} placeholder="-- Choose Reason --" icon={false} options={IDLE_REASONS[idleCategory]} onChange={(val) => { setIdleSubReason(val); if (val !== 'Other') setIdleRemarks(''); }} />
                         </div>
                       )}
                       ...
                       {idleSubReason === 'Other' && (
                         <div style={{ marginBottom: '24px', animation: 'slideDownDropdown 0.3s ease-out forwards' }}>
                           <label style={{ fontSize: '11px', color: 'var(--status-running)', fontWeight: '800', textTransform: 'uppercase', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '5px', letterSpacing: '1.5px' }}><i className="bi bi-pencil-square"></i> {idleCategory === 'Other' ? '3. Please Specify Exact Issue' : '3. Please Specify Reason'}</label>
                           <textarea rows="2" placeholder="Type the exact issue here..." value={idleRemarks} onChange={(e) => setIdleRemarks(e.target.value)} style={{ width: '100%', padding: '14px 18px', borderRadius: '12px', background: 'var(--modal-input-bg)', color: 'var(--text-main)', border: '1px solid var(--status-running)', fontSize: '14px', outline: 'none', resize: 'vertical' }} />
                         </div>
                       )}
 
                       <button onClick={handleReasonSubmit} disabled={isSubmittingReason || !isFormValid} style={{ width: '100%', padding: '16px', borderRadius: '16px', marginTop: '10px', background: (!isFormValid) ? 'var(--inner-box-bg)' : 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)', color: (!isFormValid) ? 'var(--text-dim)' : '#fff', border: (!isFormValid) ? '1px dashed var(--dropdown-border)' : 'none', fontWeight: '900', fontSize: '16px', letterSpacing: '1px', textTransform: 'uppercase', cursor: (!isFormValid) ? 'not-allowed' : 'pointer', transition: 'all 0.3s ease', boxShadow: (isFormValid) ? '0 10px 25px rgba(245, 158, 11, 0.4)' : 'none', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px' }}>
                         {isSubmittingReason ? <><div className="loader" style={{ width: '20px', height: '20px', border: '3px solid rgba(255,255,255,0.3)', borderTop: '3px solid #fff', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div> Submitting...</> : <><i className="bi bi-floppy-fill"></i> Save Reason</>}
                       </button>
                     </>
                   ) : (
                     <div style={{ textAlign: 'center', padding: '10px', color: 'var(--status-running)', fontWeight: 'bold', fontSize: '14px' }}>✅ Reason has been successfully recorded for this session. It will reset when the machine starts running again.</div>
                   )}
                 </div>
               )}
 
               {isCurrentlyLunch && !selectedMachine.is_producing && (
                 <div style={{ background: 'rgba(59, 130, 246, 0.15)', border: '1px solid rgba(96, 165, 250, 0.4)', color: '#1d4ed8', padding: '16px', borderRadius: '12px', textAlign: 'center', fontWeight: '800', letterSpacing: '2px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', textTransform: 'uppercase', animation: 'lunchBannerPulse 3s infinite' }}>
                   <span style={{ fontSize: '24px' }}>🍽️</span> <span style={{ fontSize: '18px' }}>LUNCH TIME BREAK</span>
                 </div>
               )}
               
               <div style={{ backgroundColor: 'var(--inner-box-bg)', padding: '20px', borderRadius: '12px', border: '1px solid var(--inner-box-border)' }}>
                 <h3 style={{ margin: '0 0 16px 0', color: 'var(--status-running)', display: 'flex', alignItems: 'center', gap: '8px' }}><span>⏱️</span> Live Session Timings</h3>
                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                   <div style={{ padding: '16px', background: 'var(--stat-run-bg)', borderRadius: '12px', display: 'flex', justifyContent: 'space-between' }}><div><div style={{fontSize: '12px', color: 'var(--stat-run-color)'}}>Machine Turned ON</div><div style={{fontSize: '20px', fontWeight: '700', color: 'var(--stat-run-color)'}}>{selectedMachine.on_since || '--:--'}</div></div><div style={{textAlign: 'right'}}><div style={{fontSize: '20px', fontWeight: '700', color: 'var(--stat-run-color)'}}>{selectedMachine.on_duration_minutes || 0}</div><div style={{fontSize: '10px', color: 'var(--stat-run-color)'}}>minutes ago</div></div></div>
                   <div style={{ padding: '16px', background: 'var(--stat-tot-bg)', borderRadius: '12px', display: 'flex', justifyContent: 'space-between' }}><div><div style={{fontSize: '12px', color: 'var(--stat-tot-color)'}}>First Count Received</div><div style={{fontSize: '20px', fontWeight: '700', color: 'var(--stat-tot-color)'}}>{selectedMachine.first_count_at || '--:--'}</div></div><div style={{textAlign: 'right'}}><div style={{fontSize: '20px', fontWeight: '700', color: 'var(--stat-tot-color)'}}>{selectedMachine.time_to_first_count || 0}</div><div style={{fontSize: '10px', color: 'var(--stat-tot-color)'}}>min delay</div></div></div>
                   <div style={{ padding: '16px', background: 'var(--stat-idle-bg)', borderRadius: '12px', display: 'flex', justifyContent: 'space-between' }}><div><div style={{fontSize: '12px', color: 'var(--stat-idle-color)'}}>Last Count This Hour</div><div style={{fontSize: '20px', fontWeight: '700', color: 'var(--stat-idle-color)'}}>{selectedMachine.last_activity || 'Never'}</div></div><div style={{textAlign: 'right'}}><div style={{fontSize: '16px', fontWeight: '700', color: 'var(--stat-idle-color)'}}>{selectedMachine.current_hour_count} counts</div></div></div>
                 </div>
                 <div style={{ marginTop: '16px', padding: '16px', background: 'var(--dropdown-bg)', borderRadius: '8px', border: '1px solid var(--dropdown-border)', display: 'flex', justifyContent: 'space-between' }}><span style={{ color: 'var(--text-muted)' }}>Status</span><span style={{ fontWeight: 'bold', color: selectedMachine.is_producing ? 'var(--status-running)' : 'var(--status-idle)' }}>{selectedMachine.is_producing ? '✅ Currently Producing' : '⚠️ Idle / Offline'}</span></div>
               </div>
               <div style={{ textAlign: 'center', padding: '30px', background: 'var(--card-bg)', borderRadius: '20px', border: `1px solid var(--card-border)`, marginTop: '24px', boxShadow: `var(--card-shadow)` }}><div style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '8px', fontWeight: '800', textTransform: 'uppercase' }}>Current Hour Production</div><div style={{ fontSize: '72px', fontWeight: '900', color: getMachineColor(selectedMachine), textShadow: `0 0 30px ${getMachineColor(selectedMachine)}40`, lineHeight: '1' }}>{selectedMachine.current_hour_count || 0}</div><div style={{ fontSize: '12px', color: 'var(--text-dim)', marginTop: '10px', fontWeight: '600' }}>Resets every hour at XX:00:00</div></div>
               <div style={{ backgroundColor: 'var(--inner-box-bg)', padding: '20px', borderRadius: '12px', border: '1px solid var(--inner-box-border)', marginTop: '24px' }}>
                 <h3 style={{ margin: '0 0 16px 0', color: '#3b82f6', display: 'flex', alignItems: 'center', gap: '8px' }}><span>👤</span> Customer Information</h3>
                 <div style={{ display: 'grid', gap: '10px' }}>
                   <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'var(--dropdown-bg)', border: '1px solid var(--dropdown-border)', borderRadius: '8px' }}><span style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Customer</span><span style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>{selectedMachine.tool_customer || 'N/A'}</span></div>
                   <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'var(--dropdown-bg)', border: '1px solid var(--dropdown-border)', borderRadius: '8px' }}><span style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Model</span><span style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>{selectedMachine.tool_model || 'N/A'}</span></div>
                 </div>
               </div>
               <div style={{ backgroundColor: 'var(--inner-box-bg)', padding: '20px', borderRadius: '12px', border: '1px solid var(--inner-box-border)', marginTop: '24px' }}>
                 <h3 style={{ margin: '0 0 16px 0', color: 'var(--status-running)', display: 'flex', alignItems: 'center', gap: '8px' }}><span>🔧</span> Tool Information</h3>
                 <div style={{ display: 'grid', gap: '10px' }}>
                   <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'var(--dropdown-bg)', border: '1px solid var(--dropdown-border)', borderRadius: '8px' }}><span style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Part Name</span><span style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>{selectedMachine.tool_part_name || 'N/A'}</span></div>
                   <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'var(--dropdown-bg)', border: '1px solid var(--dropdown-border)', borderRadius: '8px' }}><span style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Tool Name</span><span style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>{selectedMachine.tool_name || 'N/A'}</span></div>
                   <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'var(--dropdown-bg)', border: '1px solid var(--dropdown-border)', borderRadius: '8px' }}><span style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Part Number</span><span style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>{selectedMachine.tool_part_number || 'N/A'}</span></div>
                   <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'var(--dropdown-bg)', border: '1px solid var(--dropdown-border)', borderRadius: '8px' }}><span style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Shut Height</span><span style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>{typeof selectedMachine.shut_height === 'number' ? selectedMachine.shut_height.toFixed(2) : selectedMachine.shut_height || 'N/A'}</span></div>
                   <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: 'var(--dropdown-bg)', border: '1px solid var(--dropdown-border)', borderRadius: '8px' }}><span style={{ color: 'var(--text-muted)', fontWeight: '600' }}>Tool ID</span><span style={{ fontWeight: 'bold', color: 'var(--status-running)', wordBreak: 'break-all' }}>{selectedMachine.tool_id || 'N/A'}</span></div>
                 </div>
               </div>
               <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px', marginTop: '24px' }}>
                 <div style={{ padding: '20px', background: 'var(--inner-box-bg)', borderRadius: '16px', border: '1px solid var(--inner-box-border)', textAlign: 'center' }}><div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: '800', letterSpacing: '1px', marginBottom: '10px' }}>LAST HOUR</div><div style={{ fontSize: '36px', fontWeight: '800', color: 'var(--text-main)' }}>{selectedMachine.last_hour_count || 0}</div></div>
                 <div style={{ padding: '20px', background: 'var(--inner-box-bg)', borderRadius: '16px', border: '1px solid var(--inner-box-border)', textAlign: 'center' }}><div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: '800', letterSpacing: '1px', marginBottom: '10px' }}>CUMULATIVE</div><div style={{ fontSize: '36px', fontWeight: '800', color: 'var(--status-running)' }}>{selectedMachine.cumulative_count || 0}</div></div>
                 <div style={{ padding: '20px', background: 'var(--inner-box-bg)', borderRadius: '16px', border: '1px solid var(--inner-box-border)', textAlign: 'center' }}><div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: '800', letterSpacing: '1px', marginBottom: '10px' }}>TOTAL IDLE</div><div style={{ fontSize: '36px', fontWeight: '800', color: 'var(--status-idle)' }}>{selectedMachine.total_shift_idle_time || 0} <span style={{fontSize: '14px'}}>min</span></div></div>
               </div>
             </div>
             )}
           </div>
